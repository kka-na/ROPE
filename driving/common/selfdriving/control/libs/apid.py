...
class APID:
    ...
    def set_configs(self, config):
        ...
        self.max_accel  = float(cm_config['max_accel'])
        self.max_decel = float(cm_config['max_decel'])

    def execute(self, state, target_velocity, current_velocity):
        if state < 1:
            return -3
                
        self.update_history(target_velocity, current_velocity)
        self.calculate_ddK()
        self.calculate_dK()
        ...
        if output < 0:
            output = (self.max_decel / 100) * output #Brake
        else:
            output = (self.max_accel / 100) * output #Accel
        return output
